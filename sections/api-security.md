...
- Client application identity **MUST** be established via a consistent mechanism. This may be via an API Key, client credentials (API key + secret) or via an OAuth 2.0 asserted server identity. Use of API keys should only be permitted when TLS is enabled. Rotation policy for API Key/secret should be implemented where applicable.
